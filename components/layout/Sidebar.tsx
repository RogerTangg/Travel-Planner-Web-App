    ...
    <form onSubmit={onSubmit} className="relative mb-2">
      ...
      <div className="relative mb-2">
        <input 
          ...
          className={`w-full pl-10 pr-4 py-3 md:py-2.5 bg-gray-50 border-2 rounded-xl text-sm focus:bg-white focus:ring-2 outline-none transition-all ${
            isManualMode 
      ...
      <div className="flex gap-1.5">
        <button 
          ...
          className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 md:py-2 rounded-lg shadow-sm hover:shadow-md border-2 disabled:opacity-50 font-medium text-sm transition-all active:scale-95 ${
            isManualMode 
        ...
        <button 
          ...
          className={`px-3 py-2.5 md:py-2 rounded-lg shadow-sm hover:shadow border-2 transition-all active:scale-95 ${
            isManualMode 
        ...
        <div className="relative">
          ...
          <button 
            ...
            title="上傳行程文字檔"
            className="px-3 py-2.5 md:py-2 bg-white text-gray-500 hover:text-sakura-500 rounded-lg shadow-sm hover:shadow border-2 border-gray-200 hover:border-sakura-200 disabled:opacity-50 transition-all active:scale-95"
          >
  ...
  return (
    <div className="mb-2">
      <div className="flex items-center gap-1 mb-1.5">
        ...
        <span className="text-[10px] font-medium text-gray-500">快速新增</span>
      </div>
      <div className="flex gap-1.5 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-hide">
        {QUICK_MODULES.map(module => (
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1.5 bg-white rounded-lg border border-gray-200 text-xs font-medium text-gray-600 hover:border-sakura-300 hover:bg-sakura-50 hover:text-sakura-600 active:scale-95 transition-all shadow-sm whitespace-nowrap flex-shrink-0"
          >
  ...
  return (
    <div className="flex items-center gap-1.5 mb-2">
      <button
        ...
        title="匯出行程 JSON"
        className="flex-1 flex items-center justify-center gap-1 px-2 py-2 bg-white rounded-lg border border-gray-200 text-xs font-medium text-gray-600 hover:border-green-300 hover:text-green-600 active:bg-green-50 active:scale-95 transition-all shadow-sm"
      >
        <Download size={14} />
        <span>匯出</span>
      </button>
      <button
        ...
        title="匯入行程 JSON"
        className="flex-1 flex items-center justify-center gap-1 px-2 py-2 bg-white rounded-lg border border-gray-200 text-xs font-medium text-gray-600 hover:border-blue-300 hover:text-blue-600 active:bg-blue-50 active:scale-95 transition-all shadow-sm"
      >
        <FolderInput size={14} />
        <span>匯入</span>
      </button>
      ...
      <div className="flex flex-wrap gap-1.5">
        {selectedTagFilter && (
          <button
            ...
            className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-medium bg-gray-100 text-gray-600 hover:bg-gray-200 active:scale-95 transition-all"
          >
            <X size={10} />
            清除
          </button>
        ...
        {allTags.map(tag => (
          <button
            ...
            className={`px-2 py-1 rounded-full text-[10px] font-medium transition-all active:scale-95 ${
    ...
    <div className="w-full md:w-[320px] lg:w-[340px] flex-shrink-0 bg-white border-r border-gray-200 flex flex-col z-20 shadow-[4px_0_24px_rgba(0,0,0,0.02)] h-full">
      ...
      <div className="p-4 md:p-5 border-b border-gray-100 bg-white flex-shrink-0">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <img src="/logo.svg" alt="Logo" className="w-8 h-8 md:w-7 md:h-7" />
        ...
        <input 
          ...
          className="w-full text-base font-bold text-gray-800 bg-gray-50 px-3 py-2.5 rounded-lg border-transparent focus:bg-white focus:ring-2 focus:ring-sakura-200 transition-all outline-none" 
        />
        
        {/* 天數控制 */}
        <div className="flex items-center justify-between mt-3 bg-gray-50 p-2.5 rounded-lg">
          <span className="text-xs font-medium text-gray-500 ml-1">旅遊天數</span>
          <div className="flex items-center gap-3">
            <button 
              onClick={() => updateDayCount(currentTrip.dayCount - 1)} 
              className="w-9 h-9 md:w-6 md:h-6 rounded-lg md:rounded flex items-center justify-center bg-white hover:bg-gray-100 active:bg-gray-200 text-gray-600 shadow-sm active:scale-90 transition-all font-semibold text-lg md:text-base"
              aria-label="減少天數"
            >
              −
            </button>
            <span className="text-lg md:text-sm font-bold w-6 text-center">{currentTrip.dayCount}</span>
            <button 
              onClick={() => updateDayCount(currentTrip.dayCount + 1)} 
              className="w-9 h-9 md:w-6 md:h-6 rounded-lg md:rounded flex items-center justify-center bg-white hover:bg-gray-100 active:bg-gray-200 text-gray-600 shadow-sm active:scale-90 transition-all font-semibold text-lg md:text-base"
              aria-label="增加天數"
